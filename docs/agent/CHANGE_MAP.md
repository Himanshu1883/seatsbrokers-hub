...
| Journey in numbers | `landing/JourneyNumbers.tsx` | `.journey-num*`. Desktop is content-height: no `100dvh` / `100svh` / 340vh pin. Section `padding-block: 3.25rem` (≥1024px; over-tight cut was `2.5rem`; original was `100dvh` well). Mobile stack `2.75rem 3rem` (over-tight `2rem 2.25rem`; original `3rem 3.5rem`). Header in-flow (`width: 100%`, not absolute over a viewport well). Stage `min-height: clamp(14rem, 22vh, 16.5rem)` (was `13rem / 34vh / 20rem`). Viewport `overflow: hidden` (contains the 1.85 enter scale; not a scroll pane). Section `overflow: visible` (no `overflow-x-clip` wheel trap). In-view autoplay + reduced-motion stack unchanged. `.sto-*` / `.tpa-*` / `.mps-*` / `.toolkit-*` / FeatureOrbit / Nav untouched. |
