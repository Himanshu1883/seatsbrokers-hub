...
Hero/console datasets: `src/content/{broker,travel,marketplace,event-intel,api}-hero-data.ts`, `inventory-console-data.ts`, `seat-map-tickets-data.ts`. FAQ: `faq-data.ts`. Legal: `legal-data.ts`.
