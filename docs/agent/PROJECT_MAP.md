...
| Platform operating ecosystem | `src/content/platform-page-data.ts` → `PlatformHero` + `PlatformStackWall` (dark ConsoleShell spine) + `PlatformModuleMap` (7-up `.plt-mod-*` + `PlatformStageBoard` hero/consoles); `.plt-os-*` / `.plt-mod-*` / `.plt-stage-*` append in `styles.css` |
