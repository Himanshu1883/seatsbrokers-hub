...
5. **SectionConnector 02** — Product APIs → Real-time sync (tone `dark`; payload inventory update, listing status, order event, delivery)
6. **ApiLiveConsole `webhooks`** — `WebhookDeliveryConsole` (terminal chrome, dark band): sign → POST → retry → ack pipeline, event payload explorer (`inventory.updated` / `listing.live` / `order.confirmed` / `delivery.transferred`), delivery attempts, delivery log. Not a 1:1 clone of the hero docs console.
7. **WorkflowSteps** — Your systems → SeatsBrokers APIs → Inventory, listings and orders
...
- Console CSS prefixes: `apidoc-` hero docs, `apk-` auth, `whk-` webhooks — appended as a marked block at the end of `src/styles.css`
